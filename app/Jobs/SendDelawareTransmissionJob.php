...
use App\Services\DelawareDmvService;
...
class SendDelawareTransmissionJob implements ShouldQueue
{
     ...
     * Execute the job.
     */
    public function handle(DelawareDmvService $dmvService): void
    {
        Log::info("Processing Delaware DMV transmission", [
        ...
        try {
            ...
            // Submit to Delaware DMV
            $result = $dmvService->submitCertificate($this->transmission->payload_json);
            ...
            if ($result['success']) {
                ...
                $this->transmission->update([
                    ...
                    'response_message' => $result['message'] ?? 'Successfully submitted to Delaware DMV',
                    'sent_at' => now(),
                ]);
                ...
                Log::info("Delaware DMV transmission successful", [
                    'transmission_id' => $this->transmission->id,
            ...
            Log::error("Delaware DMV transmission exception", [
                'transmission_id' => $this->transmission->id,
                'error' => $e->getMessage(),
    ...
    {
        Log::error("Delaware DMV transmission job failed permanently", [
            'transmission_id' => $this->transmission->id,
    ...
    private function handleFailure(string $error, string $responseCode): void
    {
        ...
        if (in_array($responseCode, $retryableErrors) && $this->attempts() < $this->tries) {
            Log::info("Delaware DMV transmission will be retried", [
                'transmission_id' => $this->transmission->id,
            ...
            Log::error("Delaware DMV transmission failed permanently", [
                'transmission_id' => $this->transmission->id,
                'error' => $error,
                'response_code' => $responseCode
            ]);
