...
use App\Services\FloridaDicdsService;
...
class SendFloridaTransmissionJob implements ShouldQueue
{
     ...
     * Execute the job.
     */
    public function handle(FloridaDicdsService $dicdsService): void
    {
        Log::info("Processing Florida DICDS transmission", [
        ...
        try {
            ...
            // Submit to Florida DICDS
            $result = $dicdsService->submitCertificate($this->transmission->payload_json);
            ...
            if ($result['success']) {
                ...
                $this->transmission->update([
                    ...
                    'response_message' => $result['message'] ?? 'Successfully submitted to DICDS',
                    'sent_at' => now(),
                ]);
                ...
                Log::info("Florida DICDS transmission successful", [
                    'transmission_id' => $this->transmission->id,
            ...
            Log::error("Florida DICDS transmission exception", [
                'transmission_id' => $this->transmission->id,
                'error' => $e->getMessage(),
    ...
    {
        Log::error("Florida DICDS transmission job failed permanently", [
            'transmission_id' => $this->transmission->id,
    ...
    private function handleFailure(string $error, string $responseCode): void
    {
        ...
        if (in_array($responseCode, $retryableErrors) && $this->attempts() < $this->tries) {
            Log::info("Florida DICDS transmission will be retried", [
                'transmission_id' => $this->transmission->id,
            ...
            Log::error("Florida DICDS transmission failed permanently", [
                'transmission_id' => $this->transmission->id,
                'error' => $error,
                'response_code' => $responseCode
            ]);
