...
use App\Services\TexasTdlrService;
...
class SendTexasTransmissionJob implements ShouldQueue
{
     ...
     * Execute the job.
     */
    public function handle(TexasTdlrService $tdlrService): void
    {
        Log::info("Processing Texas TDLR transmission", [
        ...
        try {
            ...
            // Submit to Texas TDLR
            $result = $tdlrService->submitCertificate($this->transmission->payload_json);
            ...
            if ($result['success']) {
                ...
                $this->transmission->update([
                    ...
                    'response_message' => $result['message'] ?? 'Successfully submitted to Texas TDLR',
                    'sent_at' => now(),
                ]);
                ...
                Log::info("Texas TDLR transmission successful", [
                    'transmission_id' => $this->transmission->id,
            ...
            Log::error("Texas TDLR transmission exception", [
                'transmission_id' => $this->transmission->id,
                'error' => $e->getMessage(),
    ...
    {
        Log::error("Texas TDLR transmission job failed permanently", [
            'transmission_id' => $this->transmission->id,
    ...
    private function handleFailure(string $error, string $responseCode): void
    {
        ...
        if (in_array($responseCode, $retryableErrors) && $this->attempts() < $this->tries) {
            Log::info("Texas TDLR transmission will be retried", [
                'transmission_id' => $this->transmission->id,
            ...
            Log::error("Texas TDLR transmission failed permanently", [
                'transmission_id' => $this->transmission->id,
                'error' => $error,
                'response_code' => $responseCode
            ]);
