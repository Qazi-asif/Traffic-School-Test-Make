...
class QuizController extends Controller
{
    ...
    public function show($chapterId)
    {
        ...
        if (!$progress || !$progress->started_at) {
            return redirect()->route('student.florida.courses.show', $chapter->course_id)
        ...
        return view('student.florida.quiz.show', compact(
            'chapter',
            'questions',
            'enrollment',
            'progress',
    ...
    public function submit(Request $request, $chapterId)
    {
        ...
        return response()->json([
            ...
            'redirect' => route('student.florida.quiz.results', $quizResult->id),
        ]);
    }
    ...
    public function results($resultId)
    {
        ...
        return view('student.florida.quiz.results', compact(
            'quizResult',
            'answers',
            'questions'
        ));
    ...
    public function retry($chapterId)
    {
        ...
        return redirect()->route('student.florida.quiz.show', $chapterId)
    ...
    {
        // Florida allows unlimited retakes for failed quizzes
        $maxAttempts = $chapter->course->max_quiz_attempts ?? 999;
