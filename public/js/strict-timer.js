...
class StrictTimer {
    ...
    bindEvents() {
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.hidden && !window.isAdmin) { // Skip for admin users
                this.recordViolation('tab_switch', {
                    timestamp: Date.now(),
                    type: 'visibility_hidden'
                });
                this.tabSwitches++;
            } else if (document.hidden && window.isAdmin) {
                console.log('👨‍💼 Admin user - bypassing tab switch violation');
            }
        });

        // Detect window blur/focus
        window.addEventListener('blur', () => {
            if (!window.isAdmin) { // Skip for admin users
                ...
                console.log('👨‍💼 Admin user - bypassing window blur violation');
            }
        });
        ...
        setInterval(() => {
            // Skip devtools detection for admin users
            if (window.isAdmin) {
                return;
            }
        ...
        document.addEventListener('keydown', (e) => {
            if (this.isActive && !window.isAdmin) { // Skip for admin users
    ...
    preventBypassMethods() {
        // Disable text selection (but allow for admin users)
        document.onselectstart = () => (this.isActive && !window.isAdmin) ? false : true;
        document.ondragstart = () => (this.isActive && !window.isAdmin) ? false : true;

        // Override console methods to detect script injection (skip for admin users)
        const originalLog = console.log;
        console.log = (...args) => {
            if (this.isActive && !window.isAdmin && args.some(arg => 
                typeof arg === 'string' && 
    ...
    async startTimer(chapterId, enrollmentId = null, chapterDurationMinutes = null, enforceMinimumTime = false, requiredMinTime = null) {
        ...
        try {
            ...
            this.enforceMinimumTime = enforceMinimumTime;
            this.requiredMinTime = requiredMinTime ? requiredMinTime * 60 : null; // Convert to seconds
            
            const requestData = {
                chapter_id: parseInt(chapterId), // Convert to integer
                browser_fingerprint: this.browserFingerprint,
                enrollment_id: enrollmentId ? parseInt(enrollmentId) : null // Convert to integer if provided
            };
    ...
    startCountdown() {
        ...
        this.interval = setInterval(() => {
            ...
            // If enforce_minimum_time is enabled, use required_min_time instead of duration
            const timerLimit = this.enforceMinimumTime && this.requiredMinTime ? this.requiredMinTime : this.requiredTime;
            if (this.elapsedTime >= timerLimit) {
                this.completeTimer();
            }
        }, 1000);
    }
    ...
    recordViolation(type, details) {
        // Check if user is admin and bypass violations
        if (window.isAdmin) {
            console.log(`👨‍💼 Admin user - bypassing timer violation: ${type}`, details);
            return;
        }
    ...
    updateDisplay() {
        ...
        // Update action buttons if function exists (this will update quiz button state)
        if (typeof updateActionButtons === 'function') {
            updateActionButtons();
        }
        
        // Force button update when timer completes
        if (this.elapsedTime >= this.requiredTime) {
            setTimeout(() => {
                if (typeof updateActionButtons === 'function') {
                    updateActionButtons();
    ...
    completeTimer() {
        ...
        // Update display one final time to show 0:00 and enable button
        this.updateDisplay();
        
        // Set isActive to false AFTER updating display
        this.isActive = false;
    ...
    isTimerComplete() {
        // Timer is complete if elapsed time >= required time
